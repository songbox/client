...
test('should have a sidebar with items', async function (assert) {
  ...
  assert.equal(page.sidebar.main.items().count, 2);
});
...
test('sidebar item should navigate to song and select it', async function (assert) {
  ...
  assert.equal(page.sidebar.main.items().count, 2, 'has 2 songs in list');
...
test('sidebar should enable removal of songs', async function (assert) {
  ...
  assert.equal(page.sidebar.main.items().count, 2, '2 songs in the list');

  await page.sidebar.main.items(0).buttons.remove();

  assert.equal(page.sidebar.main.items().count, 1, 'only 1 song in the list');
